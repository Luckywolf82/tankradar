...
const classifyAndPrioritize = (station) => {
  const nameNorm = norm(station.name);
  
  // Detect marine signals
  ...
  // stationType = marine_fuel is highest priority
  ...
  // Strong marine + fuel adjacent keywords
  ...
  // Strong fuel-adjacent keywords alone (oljeservice, etc)
  ...
  // Marine keywords alone (less than 2)
  ...
  // No strong signals - unclear local business
  return {
...
Deno.serve(async (req) => {
  ...
  console.log('[buildChainUnconfirmedManualPriorityQueue] Starting queue build...');
  ...
  console.log(`[buildChainUnconfirmedManualPriorityQueue] Found ${pendingReviews.length} pending chain_unconfirmed reviews`);

  const queueItems = [];
  const examplesByPriority = {
    high: [],
    medium: [],
    low: [],
  };

  const countsByPriority = { HIGH: 0, MEDIUM: 0, LOW: 0 };
  const countsBySemanticCategory = {
    likely_marine_or_fuel_adjacent: 0,
    likely_service_or_trade_site: 0,
    unclear_named_local_business: 0,
  };
  const countsByRecommendedAction = {
    inspect_as_possible_marine_fuel: 0,
    inspect_as_possible_local_fuel_site: 0,
    inspect_as_service_or_trade: 0,
    inspect_as_foreign_or_border_case: 0,
    leave_for_manual_name_lookup: 0,
  };
  ...
  for (const review of pendingReviews) {
    ...
    countsByPriority[classification.priorityLevel]++;
    countsBySemanticCategory[classification.semanticCategory]++;
    countsByRecommendedAction[classification.recommendedManualAction]++;
    ...
    // Keep first 10 examples per priority
    const priorityKey = classification.priorityLevel.toLowerCase();
    if (examplesByPriority[priorityKey].length < 10) {
      examplesByPriority[priorityKey].push(item);
    }
  }

  // Sort queue: HIGH → MEDIUM → LOW, then by confidence descending, then by name
  queueItems.sort((a, b) => {
  ...
  const summary = {
    ...
    countsByPriority,
    countsBySemanticCategory,
    countsByRecommendedAction,
  };

  console.log('[buildChainUnconfirmedManualPriorityQueue] ── QUEUE BUILD SUMMARY ──');
  console.log(`  Total analyzed: ${summary.totalAnalyzed}`);
  console.log(`  Priorities: HIGH=${countsByPriority.HIGH}, MEDIUM=${countsByPriority.MEDIUM}, LOW=${countsByPriority.LOW}`);
  console.log(`  Semantic categories:`, countsBySemanticCategory);
  console.log('[buildChainUnconfirmedManualPriorityQueue] Queue build complete');
