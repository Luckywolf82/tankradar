...
Deno.serve(async (req) => {
  ...
  console.log('[previewResolveSemanticChainUnconfirmed] Starting preview...');
  ...
  console.log(`[previewResolveSemanticChainUnconfirmed] Found ${pendingReviews.length} pending chain_unconfirmed reviews`);
  ...
  for (const review of pendingReviews) {
    ...
    if (!safe || !targetType) {
      toSkip.push({
        ...
        reason: `Bucket: ${bucket}, Confidence: ${confidence}, Safe: ${safe}`,
        bucket,
        confidence,
      });
      continue;
  ...
  for (const item of toReclassify) {
    const key = item.targetReviewType;
  ...
  const summary = {
    ...
    totalReclassified: toReclassify.length,
    totalSkipped: toSkip.length,
    reclassifiedByTargetType: reclassifiedByType,
  };

  console.log('[previewResolveSemanticChainUnconfirmed] ── PREVIEW SUMMARY ──');
  console.log(`  Total analyzed: ${summary.totalAnalyzed}`);
  console.log(`  Safe to reclassify: ${summary.totalReclassified}`);
  ...
  return Response.json({
    ...
    dryRun: true,
    summary,
    toReclassify: toReclassify.slice(0, 20),
    toSkip: toSkip.slice(0, 10),
    allToReclassify: toReclassify,
  });
});