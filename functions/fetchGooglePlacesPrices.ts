...
// NOTE: Google uses Money protobuf: units = whole NOK, nanos = 10^-9 NOK
// LOCKED FORMULA: priceNok = units + (nanos / 1e9)
// e.g., units=23, nanos=500000000 → 23 + 0.5 = 23.50 NOK/L
      ...
      for (const googlePlace of googleResult.places) {
        ...
        for (const fuelPrice of fuelOptions) {
          ...
          await base44.entities.FuelPrice.create({
            ...
            rawPayloadSnippet: `${fuelPrice.type} ${priceNok} NOK [${plausibilityStatus}]`
          });
