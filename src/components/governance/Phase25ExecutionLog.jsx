...
### Alert System Status (Canonical Path)
**Phase 6A (Primary):** PriceAlert entity
- Model: Geolocation-based (latitude, longitude, radiusKm)
- Matching: Any FuelPrice with matching fuelType, price ≤ maxPrice, within radius
- Backend: checkPriceAlerts.js (service-role, creates PriceAlertEvent on match)
- Frontend: PriceAlerts.jsx (create, enable/disable, delete alerts; view triggered events)
- Notifications: Events stored only, no push/email
- Premium Gating: Not enforced
- Status: Fully implemented and active

**Alternate (Not Exposed):** UserPriceAlert entity
- Model: Station-specific (tied to single station ID)
- Matching: Station ID + fuel type + alert type (below_user_target, price_drop, etc.)
- Backend: createUserPriceAlert.js (premium-gated), checkPriceDropAlerts.js (automation)
- Frontend: Not exposed in current UI
- Notifications: Events stored in alert record, no push/email
- Premium Gating: Enforced (user.role === 'premium')
- Status: Implemented but not active in user-facing UI
