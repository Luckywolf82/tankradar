...
import { ArrowRight, CheckCircle2, AlertTriangle, ClipboardList, MapPin, Database, Loader2 } from "lucide-react";
...
export default function AdminStartHer({ onNavigate }) {
  ...
  useEffect(() => {
    ...
    ]).then(([reviewItems, discoveryItems, logs]) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          {loading ? (
            ...
            <>
              ...
              <p className="text-xs text-amber-600 mt-1">
                {reviewCount === 0
                  ? "Ingen saker venter — bra!"
                  : reviewCount === 1 ? "sak venter på behandling" : "saker venter på behandling"}
              </p>
            </>
          )}
          <p className="text-xs text-amber-500 mt-2 flex items-center gap-1 group-hover:text-amber-700">
            Gå til Behandle saker <ArrowRight size={11} />
          </p>
        </button>
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider text-purple-600">Uavklarte stasjoner</span>
          </div>
          {loading ? (
            <Loader2 size={16} className="animate-spin text-slate-400" />
          ) : (
            <>
              <p className="text-3xl font-bold text-purple-700">{discoveryCount}</p>
              <p className="text-xs text-purple-600 mt-1">
                {discoveryCount === 0 ? "Ingen uavklarte poster" : "poster uten stasjonsmatch"}
              </p>
            </>
          )}
        ...
        >
          ...
          {loading ? (
            ...
            <>
              <p className="text-3xl font-bold text-red-600">{failedSources.length}</p>
              <p className="text-xs text-red-600 mt-1">kilde{failedSources.length !== 1 ? "r" : ""} med feil</p>
            </>
          ) : (
            <>
              ...
              <p className="text-xs text-slate-500 mt-1">Siste henting vellykket</p>
            </>
          )}
          <p className="text-xs text-slate-400 mt-2 flex items-center gap-1 group-hover:text-slate-600">
            Gå til Drift <ArrowRight size={11} />
          </p>
        </button>
      </div>
