          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {MOCK_CANDIDATES.map((c, i) => (
              ...
              >
                ...
                <p className="text-xs text-slate-500">{c.chain ?? <span className="italic text-slate-400">Chain unknown</span>}</p>
                <p className="text-xs text-slate-400">{c.address}</p>
                <div className="flex gap-2 mt-1 flex-wrap">
                  <span className="text-xs bg-slate-200 text-slate-600 rounded px-1.5 py-0.5">{c.sourceCount} source{c.sourceCount !== 1 ? "s" : ""}</span>
                ...
                <div className="mt-1">
                  {c.isCanonicalExample ? (
                    <span className="text-xs font-medium bg-green-100 text-green-700 border border-green-300 rounded px-2 py-0.5">Example canonical choice</span>
                  ) : (
                    <span className="text-xs text-slate-400 bg-slate-100 border border-slate-200 rounded px-2 py-0.5">Preview only</span>
                  )}
                </div>
              </div>
            ))}
